...
class AnalyticsNginxAccessHostCounts extends AbstractMigration
{
    public function up()
    {
        $this->execute(<<<SQL
            CREATE TABLE IF NOT EXISTS `analytics_nginx_access_host_counts` (
                ...
                PRIMARY KEY `analytics_nginx_access_host_counts_pk` (`channel`, `unit`, `timestamp`)
            )
            COLLATE='utf8mb4_bin'
            ENGINE=InnoDB;
        SQL);
    ...
    {
        $this->execute('DROP TABLE `analytics_nginx_access_host_counts`');
    }
}
