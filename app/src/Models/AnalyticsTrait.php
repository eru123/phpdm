...
{
    public function parseAnalytics(array $data, string $channelKey = null, string $valueKey = null)
    {
        if (!isset($data['id'])) {
            $data['id'] = 0;
        } else {
    ...
    public function insertUpdateAnalytics(array $data, string $channelKey = null, string $valueKey = null): bool
    {
        $data = $this->parseAnalytics($data, $channelKey, $valueKey);
        $table = $this->table;

        $sql = MySql::raw(<<<SQL
            INSERT INTO `?` (`channel`, `value`, `unit`, `timestamp`)
            VALUES (?, ?, ?, ?)
